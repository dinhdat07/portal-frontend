    ...
    <div class="stack-lg">
      ...
      <article class="panel section-card">
        <form class="form-grid" [formGroup]="filtersForm" (ngSubmit)="applyFilters()">
          ...
          <div class="btn-row">
            <button type="submit" class="btn btn-primary" id="admin-apply-filters">Apply filters</button>
      ...
      <article class="panel section-card" *ngIf="!loading() && !error()">
        <div class="empty" *ngIf="users().length === 0"><h3>No users matched the current filters</h3><p>Try relaxing the filters or clearing the deleted-user constraint.</p></div>
...
export class AdminUsersPageComponent {
  ...
  confirmRestore(): void {
    ...
    if (!target || this.restoreLoading()) return;
    this.restoreLoading.set(true);
    this.adminUsersApi.restoreAdminUser(target.id).subscribe({
      next: () => { this.restoreLoading.set(false); this.restoreTarget.set(null); this.loadUsers(this.currentFilters()); },
      error: (e: unknown) => { this.restoreLoading.set(false); this.error.set(getErrorMessage(e, 'Unable to restore user')); },
    });
  }
  ...
  private loadUsers(filters: UserFilters): void {
    ...
    this.adminUsersApi.getAdminUsers(filters).subscribe({
      next: (r) => { this.loading.set(false); this.users.set(r.data); this.meta.set(r.meta); },
      error: (e: unknown) => { this.loading.set(false); this.error.set(getErrorMessage(e, 'Unable to load users')); },
    });
  }
