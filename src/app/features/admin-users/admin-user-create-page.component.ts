    ...
    <div class="stack-lg">
      ...
      <article class="panel section-card">
        ...
        <form class="form-grid" [formGroup]="form" (ngSubmit)="submit()" style="margin-top: var(--space-4);">
          ...
          <label class="field"><span>Role</span><select formControlName="role"><option value="user">user</option><option value="admin">admin</option></select><small class="field-error" *ngIf="fieldError('role')">{{ fieldError('role') }}</small></label>
          <div class="btn-row">
...
export class AdminUserCreatePageComponent {
  ...
  constructor(private readonly fb: NonNullableFormBuilder, private readonly adminUsersApi: AdminUsersApiService, private readonly router: Router) {
  ...
  submit(): void {
    ...
    this.adminUsersApi.createAdminUser({ email: v.email, username: v.username, first_name: v.firstName, last_name: v.lastName, dob: v.dob, role: v.role }).subscribe({
  ...
  fieldError(name: 'email' | 'username' | 'firstName' | 'lastName' | 'dob' | 'role'): string | null {
    ...
    if (c.hasError('required')) { if (name === 'firstName') return 'First name is required'; if (name === 'lastName') return 'Last name is required'; if (name === 'dob') return 'Date of birth is required'; if (name === 'role') return 'Role is required'; return `${name.charAt(0).toUpperCase()}${name.slice(1)} is required`; }
