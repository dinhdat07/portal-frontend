    ...
    <div class="stack-lg">
      ...
      <article class="panel section-card" *ngIf="!loading()">
        <form class="form-grid" [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div class="btn-row">
            <button type="submit" class="btn btn-primary" id="profile-save-btn" [disabled]="saving()">{{ saving() ? 'Saving...' : 'Save profile' }}</button>
            <a class="btn btn-secondary" routerLink="/account/profile">Cancel</a>
          </div>
        </form>
      </article>
...
export class ProfileEditPageComponent {
  ...
  constructor(private readonly fb: NonNullableFormBuilder, private readonly usersApi: UsersApiService, private readonly authState: AuthStateService, private readonly router: Router) {
    document.title = 'Edit Profile | Portal';
    this.loadProfile();
  }
  ...
  private loadProfile(): void {
    ...
    this.usersApi.getMyProfile().subscribe({
      next: (user) => { this.loading.set(false); this.form.setValue({ username: user.username, firstName: user.firstName, lastName: user.lastName, dob: toDateInputValue(user.dob) }); },
      error: (e: unknown) => { this.loading.set(false); this.loadError.set(getErrorMessage(e, 'Unable to load your profile')); },
    });
  }
  ...
  submit(): void {
    ...
    this.usersApi.updateMyProfile({ username: v.username, first_name: v.firstName, last_name: v.lastName, dob: v.dob }).subscribe({
      next: (user) => { this.saving.set(false); this.authState.updateUser(user); this.router.navigateByUrl('/account/profile'); },
      error: (e: unknown) => { this.saving.set(false); this.submitError.set(getErrorMessage(e, 'Unable to update profile')); },
    });
  }
